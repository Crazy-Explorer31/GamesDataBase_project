...
SELECT GameUnique.GameName, Games.NumberPlayers AS NumberOfUsers
FROM GameUnique
    JOIN Games
    ON GameUnique.GameId = Games.GameId
ORDER BY Games.NumberPlayers DESC;
...
SELECT u.UserId, u.Nickname, p.NumberHours, u.Balance,
...
SELECT u.UserId, u.Nickname, p.GameId, g.GameName
FROM Users u
    JOIN Profiles p
    ON u.UserId = p.UserId
    JOIN GameUnique g
    ON p.GameId = g.GameId
    JOIN AveragedBalance ab
    ON u.Balance > ab.AvgBalance
ORDER BY u.UserId;
...
WITH AveragedGameCost AS (
    SELECT AVG(GameCost) AS AvgGameCost
    FROM Games
)
SELECT cu.CompanyName, g.GameId, g.GameCost
FROM Companies c
         JOIN Games g ON c.CompanyId = g.CompanyId
         JOIN CompanyUnique cu ON c.CompanyId = cu.CompanyId
WHERE g.GameCost < (SELECT AvgGameCost FROM AveragedGameCost)
ORDER BY g.GameCost;
...
WITH AveragedWorkers AS (
    SELECT AVG(WorkersNumber) AS AvgWorkers
    FROM Companies
)
SELECT cu.CompanyName, c.WorkersNumber
FROM Companies c
         JOIN CompanyUnique cu ON c.CompanyId = cu.CompanyId
WHERE c.WorkersNumber > (SELECT AvgWorkers FROM AveragedWorkers)
ORDER BY c.WorkersNumber;
